class Solution {
    ...
    isAnagram(s: string, t: string): boolean {
        ...
        for(let i=0; i<s.length; i++) {
            anagramSMap.set(s[i], (anagramSMap.get(s[i]) ?? 0) + 1);
            anagramTMap.set(t[i], (anagramTMap.get(t[i]) ?? 0) + 1);
        }

        for(const [key, value] of anagramSMap) {
            if(value !== (anagramTMap.get(key) ?? 0)) {
                return false;
            }
        }
        return true;
